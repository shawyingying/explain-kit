...
1. **确认需求文档（必须问用户，不要自作主张）**：即便 demo 文件夹里有详细的 README / docs / `.md`，也**不要自动当成需求文档**——demo 说明文档 ≠ 需求文档。先问用户「哪个文件是需求文档 / 直接粘贴内容 / 跳过按 demo 现状推断」，拿到明确答复再读。**缺文档且用户让跳过时，只能按 demo 现有界面与文案推断，不要臆造需求**。
   - **提问方式按工具**：Claude Code 用 `AskUserQuestion`；Codex / Cursor 等没有该工具的，直接在对话里提问并**停下来等用户回答**，不要替用户决定、不要默认走某条路。
   - **有多个需求要讲解时**（无论来自一份还是多份文档），都要问用户「逐个生成」还是「一次性生成」并给利弊——逐个：每条可单独验证、出错只影响一条、可中途调写法，但来回多、慢；一次性：快、能通盘去重/排序步骤，但配置文件大、一条错全盘受影响、难逐条复核。建议默认逐个跑通第一条再批量补齐。Codex / Cursor 同样停下等回答。
